...
<head>
    ...
    <title>Iuran - Bendahara - AKURAD.APP</title>
...
<div class="main-content">
    <!-- Topbar hanya untuk desktop -->
    <div class="topbar d-none d-md-flex mb-2">
        ...
        <div class="top-bar-right ms-auto">
            <div class="d-flex gap-3">
                ...
                <button class="btn btn-input" data-bs-toggle="modal" data-bs-target="#modalTambahIuran" style="width: 200px;">
                    Input Iuran
                </button>
            </div>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-sm btn-outline-primary"
                                        onclick="editIuran(<?= $row['id_iuran']; ?>, '<?= $row['nik'] ?? ''; ?>', '<?= addslashes($row['nama_lengkap']); ?>', '<?= $row['rt_number'] ?? ''; ?>', '<?= $bulan_indo[$row['bulan']-1]; ?>', '<?= $row['tahun']; ?>', '<?= $row['jumlah_iuran']; ?>', '<?= $row['status_pembayaran']; ?>')">
                                    <i class="bi bi-pencil"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-success"
                                        data-bs-toggle="modal" data-bs-target="#modalKonfirmasiBayar"
                                        data-id="<?= $row['id_iuran']; ?>" data-nama="<?= addslashes($row['nama_lengkap']); ?>">
                                    <i class="bi bi-check-circle"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-danger"
                                        data-bs-toggle="modal" data-bs-target="#modalHapusIuran"
                                        data-id="<?= $row['id_iuran']; ?>">
...
<!-- Modal Tambah Iuran Baru -->
<div class="modal fade" id="modalTambahIuran" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Input Iuran Baru</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="../backend/iuran/tambah_iuran.php" method="POST">
                <div class="modal-body">
                    <p class="text-muted mb-4">Tambah data iuran untuk warga</p>

                    <!-- Pilih Warga -->
                    <div class="mb-4">
                        <label class="form-label fw-semibold mb-2">Pilih Warga</label>
                        <select class="form-select" id="id_warga" name="id_warga" required>
                            <option value="">-- Pilih Warga --</option>
                            <?php
                            $qWarga = mysqli_query($koneksi, 
                                "SELECT w.id_warga, w.nama_lengkap, u.nik 
                                 FROM warga w 
                                 JOIN user u ON w.id_warga = u.id_warga 
                                 WHERE u.role = 'warga' 
                                 ORDER BY w.nama_lengkap"
                            );
                            while ($w = mysqli_fetch_assoc($qWarga)): ?>
                                <option value="<?= $w['id_warga']; ?>">
                                    <?= htmlspecialchars($w['nama_lengkap']) . " (NIK: " . $w['nik'] . ")"; ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="row">
                            <?php foreach ($bulan_indo as $index => $bulan): ?>
                            <div class="col-md-4 mb-2">
                                <div class="form-check">
                                    <input class="form-check-input bulan-checkbox"
                                           ...
                                           id="bulan<?php echo $index; ?>">
                                    <label class="form-check-label" for="bulan<?php echo $index; ?>">
                                        <?php echo $bulan; ?>
                                    </label>
                                </div>
                            </div>
                        ...
                        <div class="alert alert-light d-flex align-items-center mt-3">
                            <i class="bi bi-info-circle me-2"></i>
                            <span id="jumlahBulanDipilih">0 bulan dipilih</span>
                        </div>
                    </div>
                    <!-- Jumlah per bulan -->
                    <div class="mb-4">
                        <label class="form-label fw-semibold mb-2">Jumlah per bulan (Rp)</label>
                        <div class="input-group mb-2">
                            <span class="input-group-text">Rp</span>
                            <input type="number" class="form-control form-control-lg" id="jumlahPerBulan" name="jumlah_per_bulan" value="150000" min="0" required>
                        </div>
                        <small class="text-muted">Total: <span id="totalJumlah">Rp 0</span></small>
                    </div>
                    
                    <!-- Tahun -->
                    <div class="mb-4">
                        <label class="form-label fw-semibold mb-2">Tahun</label>
                        <select class="form-select" id="tahunTambah" name="tahun" required>
                    ...
                    <div class="mb-4">
                        ...
                        <small class="text-muted">Biarkan kosong jika belum dibayar</small>
                    </div>
                </div>
...
<div class="modal fade" id="modalEditIuran" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="../backend/iuran/edit_iuran.php" method="POST">
                <div class="modal-body">
...
<div class="modal fade" id="modalKonfirmasiBayar" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST" action="../backend/iuran/konfirmasi.php">
                <div class="modal-body">
...
<div class="modal fade" id="modalUploadCSV" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="../backend/iuran/upload_csv.php" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const jumlahBulanDipilih = document.getElementById('jumlahBulanDipilih');
    const jumlahPerBulanInput = document.getElementById('jumlahPerBulan');
    const totalJumlahSpan = document.getElementById('totalJumlah');

    function updateJumlahBulan() {
        const checkedBoxes = document.querySelectorAll('.bulan-checkbox:checked');
        const jumlah = checkedBoxes.length;
        jumlahBulanDipilih.textContent = `${jumlah} bulan dipilih`;
    ...
    function updateTotal() {
        const checkedBoxes = document.querySelectorAll('.bulan-checkbox:checked');
        const jumlahBulan = checkedBoxes.length;
        const jumlahPerBulan = parseInt(jumlahPerBulanInput.value) || 0;
        const total = jumlahBulan * jumlahPerBulan;

        // Format ke Rupiah
        totalJumlahSpan.textContent = 'Rp ' + total.toLocaleString('id-ID');
    }

    // Event listener untuk checkbox bulan
    bulanCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', updateJumlahBulan);
    });

    // Event listener untuk input jumlah
    jumlahPerBulanInput.addEventListener('input', updateTotal);

    // Event listener untuk status radio button di modal edit
    document.querySelectorAll('.status-radio').forEach(radio => {
        radio.addEventListener('change', function() {
            // Hapus active class dari semua status option
            document.querySelectorAll('.status-option').forEach(option => {
                option.classList.remove('active');
            });

            // Tambah active class ke status option yang terpilih
            if (this.id === 'status_lunas') {
                document.querySelector('.status-option.status-lunas').classList.add('active');
            } else {
                document.querySelector('.status-option.status-belum').classList.add('active');
            }
    ...
    // Inisialisasi status radio button
    document.querySelector('.status-option.status-lunas').classList.add('active');

    // Inisialisasi pertama
    updateJumlahBulan();

    // Tahun sudah diatur otomatis di PHP dengan dropdown yang dapat dipilih
    ...
    // Event listener untuk modal konfirmasi bayar
    document.getElementById('modalKonfirmasiBayar').addEventListener('show.bs.modal', function (event) {
        ...
        document.getElementById('tanggalKonfirmasi').value = today.toISOString().split('T')[0];
    });

    // Event listener untuk modal hapus iuran
    document.getElementById('modalHapusIuran').addEventListener('show.bs.modal', function (event) {
        var button = event.relatedTarget;
        var id = button.getAttribute('data-id');
