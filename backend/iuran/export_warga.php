...
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true || $_SESSION['role'] !== 'warga') {
...
// Query data iuran dengan filter berdasarkan id_warga
$query = "SELECT
            i.id_iuran,
            i.id_warga,
            w.nama_lengkap,
...
if (!empty($filter_tahun)) {
    $query .= " AND i.tahun = '$filter_tahun'";
}
...
header('Content-Disposition: attachment; filename="data_iuran_' . preg_replace('/[^a-zA-Z0-9]/', '_', $warga_data['nama_lengkap']) . '_' . date('Y-m-d_H-i-s') . '.xls"');
...
// Header informasi
echo '<h2>Data Iuran Warga RT ' . $rt_warga . '</h2>';
