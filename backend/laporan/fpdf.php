...
protected $font;           // directory containing fonts
...
function __construct($orientation='P', $unit='mm', $size='A4')
{
	...
	// PERBAIKAN PATH FONT DI SINI - Cari folder font secara otomatis
	$fontFound = false;
	
	// Coba beberapa kemungkinan path
	$possiblePaths = array(
		__DIR__ . '/font/',                         // Dalam folder fpdf/
		dirname(__DIR__) . '/fpdf/font/',           // Satu level di atas
		dirname(dirname(__DIR__)) . '/fpdf/font/',  // Dua level di atas
		'fpdf/font/',                               // Relative path
		'../fpdf/font/',                            // Relative path ke atas
		'../../fpdf/font/',                         // Relative path dua level ke atas
	);
	
	// Tambahkan path berdasarkan lokasi file saat ini
	$currentDir = dirname($_SERVER['SCRIPT_FILENAME']);
	$possiblePaths[] = $currentDir . '/fpdf/font/';
	$possiblePaths[] = $currentDir . '/../fpdf/font/';
	$possiblePaths[] = $currentDir . '/../../fpdf/font/';

	// Coba setiap path sampai ketemu
	foreach ($possiblePaths as $path) {
		if (file_exists($path . 'helvetica.php')) {
			$this->font = $path;
			$fontFound = true;
			break;
		}
	...
	if (!$fontFound) {
		...
		$possibleFont = array(
			'fpdf/font/',                           // Path default
			dirname(__FILE__) . '/font/',           // Path relatif dari file ini
			__DIR__ . '/font/',                     // Path relatif dari direktori ini
		);

		// Tambahkan FPDF_FONTPATH jika didefinisikan
		if (defined('FPDF_FONTPATH')) {
			$possibleFont[] = constant('FPDF_FONTPATH');
		}

		// Coba setiap path
		foreach ($possibleFont as $path) {
			// Pastikan path diakhiri dengan slash
			if (substr($path, -1) !== '/' && substr($path, -1) !== '\\') {
				$path .= '/';
			}

			// Jika direktori sudah ada, gunakan ini
			if (is_dir($path)) {
				$this->font = $path;
				break;
			}

			// Jika belum ada, coba buat
		...
		// Jika masih belum ketemu, gunakan path default saja
		// Error akan terjadi saat load font jika font tidak ada
		if (!isset($this->font)) {
			$this->font = 'fpdf/font/';
		}
	}
...
{
	// PERBAIKAN: Tambahkan pengecekan file font
	if (!file_exists($path)) {
		// Coba beberapa kemungkinan path alternatif
		$filename = basename($path);
		$possiblePaths = array(
			$this->font . $filename,
			dirname(__DIR__) . '/fpdf/font/' . $filename,
			'fpdf/font/' . $filename,
			'../fpdf/font/' . $filename,
			'../../fpdf/font/' . $filename,
		);
		...
		if (!file_exists($path)) {
			$this->Error('Font file not found: ' . basename($path) . '. Please make sure the font files are in the correct directory.');
		}
	}
...
protected function _parsegif($file)
{
	...
	imagepng($im);
	$data = ob_get_clean();
	// imagedestroy is deprecated for GdImage objects in PHP 8.0+
	// GdImage objects are automatically garbage collected
	$f = fopen('php://temp','rb+');
	if(!$f)
