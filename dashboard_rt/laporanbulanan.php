...
// Prepared statement for total pemasukan - DIBATASI OLEH RT USER
$query_pemasukan = "
    SELECT COALESCE(SUM(i.jumlah_iuran), 0) as total_pemasukan
    FROM iuran_rutin i
    LEFT JOIN warga w ON i.id_warga = w.id_warga
    LEFT JOIN user u ON w.id_warga = u.id_warga
    WHERE i.bulan = ? AND i.tahun = ? AND i.status_pembayaran = 'Lunas' AND u.rt_number = '{$_SESSION['rt_number']}'
";
...
// Query untuk data transaksi bulan ini menggunakan prepared statements - DIBATASI OLEH RT USER
$query_iuran = "
    SELECT
        'iuran' as tipe,
    ...
    WHERE CAST(i.bulan AS UNSIGNED) = ? AND i.tahun = ? AND i.status_pembayaran = 'Lunas' AND u.rt_number = '{$_SESSION['rt_number']}'
";
...
mysqli_stmt_bind_param($stmt_iuran, 'ii', $bulan, $tahun);
...
$query_breakdown = "
    ...
    ORDER BY total DESC
";
$result_breakdown = mysqli_query($koneksi, $query_breakdown);
...
if (mysqli_num_rows($result_breakdown) > 0) {
    ...
    mysqli_data_seek($result_breakdown, 0); // Reset for reuse in breakdown section
}
...
<body>
            ...
            <div class="d-flex gap-3 align-items-end">
                <div class="mb-3 flex-fill">
                    ...
                    <select class="form-select" name="bulan" id="bulanSelect">
                        <option value="" <?php echo $bulan === null ? 'selected' : ''; ?>>Pilih Bulan</option>
                ...
                <div class="mb-3 flex-fill">
                    ...
                    <select class="form-select" name="tahun" id="tahunSelect">
                        <option value="" <?php echo $tahun === null ? 'selected' : ''; ?>>Pilih Tahun</option>
                        <?php
                ...
                <div class="breakdown-section">
                    ...
                    <div class="breakdown-container">
                        ...
                        if (mysqli_num_rows($result_breakdown) > 0) {
                            mysqli_data_seek($result_breakdown, 0); // Reset pointer
                            while ($row = mysqli_fetch_assoc($result_breakdown)) {
                                $color = isset($category_color_map[$row['kategori']]) ? $category_color_map[$row['kategori']] : '#6c757d';
                                echo '<div class="breakdown-item">';
                                echo '<span class="category-name">' . $row['kategori'] . '</span>';
                                echo '<span class="amount"><span class="badge" style="background-color: ' . $color . '; color: white;">' . format_rupiah($row['total']) . '</span></span>';
                                echo '</div>';
    ...
    <script>
        ...
        document.getElementById('exportExcelBtn').addEventListener('click', function() {
            ...
            const exportUrl = `../backend/laporan/exportexcel_rt.php?bulan=${bulan}&tahun=${tahun}`;
        ...
        document.getElementById('exportPdfBtn').addEventListener('click', function() {
            const bulan = document.getElementById('bulanSelect').value;
            const tahun = document.getElementById('tahunSelect').value;
            const exportUrl = `../backend/laporan/exportpdf_rt.php?bulan=${bulan}&tahun=${tahun}`;
            window.open(exportUrl, '_blank');
        });
    </script>
</body>
