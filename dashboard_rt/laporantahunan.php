...
if ($tahun !== null) {
    // Query untuk total pemasukan tahunan - DIBATASI OLEH RT USER
    $query_pemasukan = "
        ...
        WHERE i.tahun = ? AND i.status_pembayaran = 'Lunas' AND u.rt_number = '{$_SESSION['rt_number']}'
    ";
    ...
    // Query untuk data per bulan - DIBATASI OLEH RT USER
    $query_bulanan = "
        SELECT
            bulan,
        ...
        FROM (
            SELECT i.bulan, i.jumlah_iuran as jumlah, 'pemasukan' as tipe FROM iuran_rutin i
            LEFT JOIN warga w ON i.id_warga = w.id_warga
            LEFT JOIN user u ON w.id_warga = u.id_warga
            WHERE i.tahun = ? AND i.status_pembayaran = 'Lunas' AND u.rt_number = '{$_SESSION['rt_number']}'
            UNION ALL
            SELECT MONTH(p.tanggal_pengeluaran) as bulan, p.jumlah_pengeluaran as jumlah, 'pengeluaran' as tipe FROM pengeluaran_kegiatan p WHERE YEAR(p.tanggal_pengeluaran) = ?
        ) as combined
        GROUP BY bulan
        ORDER BY bulan
    ";
...
<body>
    ...
    <script>
        ...
        async function loadLaporanData(tahun) {
            try {
                const response = await fetch(`../backend/laporan/get_laporan_tahunan_rt.php?tahun=${tahun}`);
                const data = await response.json();

                if (response.ok && data.success) {
                    updateUI(data.data, tahun);
                } else {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            exportExcelBtn.addEventListener('click', function() {
                const selectedTahun = tahunSelect.value;
                if (selectedTahun) {
                    const exportUrl = `../backend/laporan/exportecxel_tahunanrt.php?tahun=${selectedTahun}`;
            ...
            exportPdfBtn.addEventListener('click', function() {
                const selectedTahun = tahunSelect.value;
                if (selectedTahun) {
                    const exportUrl = `../backend/laporan/exportpdf_tahunanrt.php?tahun=${selectedTahun}`;
                    window.open(exportUrl, '_blank');
                }
            });
        });
