...
if ($id_user) {
    ...
    if ($result_user && mysqli_num_rows($result_user) > 0) {
        ...
        // PERBAIKAN DI SINI: Gunakan isset() untuk mencegah warning
        $nama_lengkap = isset($user_data['warga_nama']) && !empty($user_data['warga_nama']) 
            ? $user_data['warga_nama'] 
            : ($user_data['username'] ?? 'Pengguna');
        $nik = $user_data['nik'] ?? '';
        ...
        $role = $user_data['role'] ?? $role; // Update role dari DB jika tersedia
    }
}
